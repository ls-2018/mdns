...
func RegisterProxy(instance, service, domain string, port int, host string, ips []string, text []string, ifaces []net.Interface) (*Server, error) {
	...
	fmt.Println(fmt.Sprintf("%s.%s", trimDot(instance), fmt.Sprintf("%s.%s.", trimDot(service), trimDot(domain))))
	entry.Port = port
	entry.Text = text
	entry.HostName = host
...
func xxx(demo string, buf []byte, n int) {
	if a {
...
func (s *Server) probe() {
	fmt.Println(time.Now().UnixNano(), s.service.ServiceInstanceName())
